...
do echo "${InstitutionArray[$i]} ${SourceArray[$i]} ${ExptArray[$i]} ${VariantArray[$i]}"
        ...
        overall_end_year=0
        for file in /badc/cmip6/data/CMIP6/ScenarioMIP/${InstitutionArray[$i]}/${SourceArray[$i]}/${ExptArray[$i]}/${VariantArray[$i]}/${file_table_id}/${file_var}/${GridArray[$i]}/latest/*.nc
        do
                echo "$file"
