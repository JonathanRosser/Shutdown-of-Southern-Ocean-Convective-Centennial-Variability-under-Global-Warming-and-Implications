...
TitleArray=["ACC/Sv", "Average Ocean Density \n South of 65$^{\circ}$S/(kgm$^{-3}$)",  "Average Ocean Density \n40-50$^{\circ}$S/(kgm$^{-3}$)","Average Density Difference \nacross the ACC/(kgm$^{-3}$)","Westerly Wind \n Max Strength/(Nm$^{-2}$)"]
...
for i in range(0,len(VariableArray)):
    ...
    at=AnchoredText(panel_labels[i],prop=dict(size=12),frameon=False,loc='upper right')
    axs[i].add_artist(at)

    plt.tight_layout()
plt.xticks(ticks=(0,20,40,60,80,100,120,140),labels=(0,20,40,60,80,100,120,140))
plt.savefig(save_location+"ACC_Forcing_Decadal_Warming_Stacked_NewLegend.pdf")
