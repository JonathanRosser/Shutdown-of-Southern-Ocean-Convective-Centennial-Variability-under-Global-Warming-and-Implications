...
VariableArray=["acc", "avgdensity65", "avgdensity40s50s", "avgdensitydiff","wwmax"]
TitleArray=["ACC std/Sv", "Average Ocean Density South of 65$^{\circ}$S std/(kgm$^{-3}$)",  "Average Ocean Density 40-50$^{\circ}$S std/(kgm$^{-3}$)","Average Density Difference \nacross the ACC/(kgm$^{-3}$)","Westerly Wind Max Strength std/(Nm$^{-2}$)"]
...
plt.close()
fig,axs=plt.subplots(len(VariableArray),sharex=True,gridspec_kw={'hspace': 0},figsize=(12,12))
for i in range(0,len(VariableArray)):
    ...
    if var_name=="avgmldd0":
        ...
        control_values=var_load(expt_id,variant_id,var_name)
        #axs[i].boxplot(control_values[850:1350],positions=[-5],widths=5,whis=(0,100))
    axs[i].set_ylabel(title_name,rotation=75,labelpad=20)
    ...
    axs[-1].legend(custom_lines,["4xCO2","1pctCO2"],ncol=2)
    axs[i].grid(visible=True,which="major")
    at=AnchoredText("("+str(i+1)+")",prop=dict(size=12),frameon=False,loc='upper right')
    axs[i].add_artist(at)


    plt.tight_layout()
plt.xticks(ticks=(0,20,40,60,80,100,120,140),labels=(0,20,40,60,80,100,120,140))
plt.savefig(save_location+"ACC_Forcing_Decadal_Warming_Stacked_Variability_NewLegend.pdf")
