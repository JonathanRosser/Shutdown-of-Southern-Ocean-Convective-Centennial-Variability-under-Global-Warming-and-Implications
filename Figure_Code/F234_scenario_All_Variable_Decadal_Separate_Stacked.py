...
def source_plot(institution_id,source_id,control_expt_id,control_variant_id,variant_list,ssp119_variant_list,ssp126_variant_list,ssp245_variant_list,ssp585_variant_list,var_name,loc_index,institution_number):
    ...
    ssp119_values=np.nanmean(data_array,axis=0)
    expt_id="ssp126"
    data=[]
    min_len=300
    ...
    ssp126_values=np.nanmean(data_array,axis=0)
    ...
    ssp245_values=np.nanmean(data_array,axis=0)
    ...
    ssp585_values=np.nanmean(data_array,axis=0)
...
for i in range(1,3):
    ...
    plt.savefig(save_location+var_name+"_Scenario_Separate_Stacked.pdf",dpi=300)
